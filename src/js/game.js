...
const createKeyBoard = () =>{
    ...
    const keyboardHTML = KEYBOARD_LETTERS.reduce((acc, curr) =>{
        return acc + `<button class="btn-primary keyboard-button" id="${curr}">${curr}</button>`
    }, '');
    ...
    if (!word.includes(inputLetter)){
        ...
        wordArray.forEach((currentLetter, index) => {
            if (currentLetter === inputLetter){
                ...
                document.getElementById(`letter_${index}`).innerText = inputLetter.toUpperCase();
            }
        }) 
    }
}
...
const stopGame = (status) =>{
    ...
    document.getElementById('game').innerHTML += `<p>The word was: <span class="result-word">${word}</span></p><button id="play-again" class="btn-primary px-5 py-2 mt-5">Play again</button>`;
...
export const startGame = () => {
    ...
    document.getElementById('quit').onclick = () =>{
        const isSure = confirm('Are you sure you want to qit and lose progress?');
        if(isSure){
            stopGame('quit');
        }
    }
